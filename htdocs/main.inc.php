...
function top_htmlhead($head, $title='', $disablejs=0, $disablehead=0, $arrayofjs='', $arrayofcss='')
{
    ...
    if (! defined('DISABLE_JQUERY') && ! $disablejs && $conf->use_javascript_ajax)
    {
        ...
        {
            $data['jquery_multiselect']=empty($conf->global->MAIN_USE_JQUERY_MULTISELECT)?constant('REQUIRE_JQUERY_MULTISELECT'):$conf->global->MAIN_USE_JQUERY_MULTISELECT;
        }
        // jQuery Timepicker
        if (! empty($conf->global->MAIN_USE_JQUERY_TIMEPICKER) || defined('REQUIRE_JQUERY_TIMEPICKER'))
        {
            $data['jquery_timepicker']=1;
        }
        // jQuery jMobile
        if (! empty($conf->global->MAIN_USE_JQUERY_JMOBILE) || defined('REQUIRE_JQUERY_JMOBILE') || (! empty($conf->dol_use_jmobile) && $conf->dol_use_jmobile > 0))
        {
        	...
        	{
        	    $data['jquery_mobile']=1;
        	}
        	if (empty($conf->dol_use_jmobile) || $conf->dol_use_jmobile != 3) $data['jquery_mobile']=2;
        }
        // jQuery jnotify
        if (empty($conf->global->MAIN_DISABLE_JQUERY_JNOTIFY) && ! defined('DISABLE_JQUERY_JNOTIFY'))
        {
            $data['jquery_notify']=1;
        }
        // jQuery blockUI
        if (! empty($conf->global->MAIN_USE_JQUERY_BLOCKUI) || defined('REQUIRE_JQUERY_BLOCKUI'))
        {
            $data['jquery_blockui']=1;
        }
        // Flot
    ...
    if (! $disablejs && ! empty($conf->use_javascript_ajax))
    {
        ...
        if (! empty($conf->fckeditor->enabled) && (empty($conf->global->FCKEDITOR_EDITORNAME) || $conf->global->FCKEDITOR_EDITORNAME == 'ckeditor'))
        {
            ...
            $ckeditor->path=$pathckeditor;
            $ckeditor->config=dol_buildpath($themesubdir.'/theme/'.$conf->theme.'/ckeditor/config.js',1);
            $ckeditor->src=$pathckeditor.$jsckeditor;
        ...
        if (! empty($conf->modules_parts['js']))		// $conf->modules_parts['js'] is array('module'=>array('file1','file2'))
        {
            ...
            foreach($arrayjs as $modjs => $filesjs)
            {
                ...
                {
                    $js[$modjs]=dol_buildpath($jsfile,1);
    ...
    $data['css_href']=$themepath.$themeparam;
    ...
    if (! empty($conf->modules_parts['css']))
    {
    	...
    	foreach($arraycss as $modcss => $filescss)
    	{
    		...
    		foreach($filescss as $cssfile)
    		{
        		...
        		$file=dol_buildpath($cssfile,1);
        		// We add params only if page is not static, because some web server setup does not return content type text/css if url has parameters, so browser cache is not used.
        		if (!preg_match('/\.css$/i',$cssfile)) $file.= $themeparam;
        		$css[$modcss]=$file;
    		}
    	}
    	$data['css']=$css;
        ...
        /* CSS by page should be done by enherit head or header.twig
         *
         * /
        
       /* JS by page should be done like css
        * 
        */
